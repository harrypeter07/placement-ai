...
app = FastAPI(title="PlaceMint Isolated Playwright Fallback Service")
...
WORKER_URL = os.getenv("TELEGRAM_WORKER_PUBLIC_URL") or os.getenv("WORKER_PUBLIC_URL") or "http://localhost:8081"
...
async def run_form_fill_job(job_id: str):
    try:
        ...
        if is_call and WORKER_URL and WORKER_SECRET:
            try:
                ...
                worker_clean_url = WORKER_URL.rstrip("/")
                requests.post(
                    f"{worker_clean_url}/send-message",
                    json={"apiKey": WORKER_SECRET, "text": message},
                    timeout=10
                )
...
@app.get("/health")
def health_check():
    job = db.get_job("dummy_id")
    return {
        "status": "healthy",
        "service": "isolated-playwright-service",
